...
from aqsp.core.time import SHANGHAI_TZ, now_shanghai
...
def analyze_debate_file(path: Path) -> dict[str, object]:
    """分析辩论结果文件。"""
    if not path.exists():
        return {"error": "文件不存在"}
    ...
    if valid_dates:
        try:
            days_oldest = (
                current_date - datetime.strptime(oldest_date, "%Y-%m-%d").date()
            ).days
            days_newest = (
                current_date - datetime.strptime(newest_date, "%Y-%m-%d").date()
            ).days
        except ValueError:
...
def clean_old_debates(
    path: Path, keep_days: int = 30, dry_run: bool = False
) -> dict[str, object]:
    """清理过期的辩论结果。"""
    if not path.exists():
        return {"error": "文件不存在"}
    ...
    cutoff_date = (now_shanghai() - timedelta(days=keep_days)).date().isoformat()
...
def main():
    ...
    parser.add_argument(
        "--file", default="data/debate_results.jsonl", help="数据文件路径"
    )
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="预览清理效果，不实际删除"
    )
    ...
    if args.analyze or args.clean:
        ...
        if "error" in analysis:
            ...
            print(
                f"   日期范围: {analysis['oldest_date']} 至 {analysis['newest_date']}"
            )
