...
SYSTEM_PROMPT = f"""你是 AQSP 的投研助理。你可以调用工具获取客观数据来支撑回答：
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "symbol": {
                        "type": "string",
                        "description": "美股字母代码 / 港股代码 / 韩股 XXXXXX.KS",
                    }
                },
                "required": ["symbol"],
            },
    ...
    try:
        ...
        if name == "query_reports":
            ...
            return [
                {
                    k: r.get(k)
                    for k in ("title", "publishDate", "orgSName", "emRatingName")
                }
                for r in rows
            ]
        if name == "query_news":
            ...
            return [
                {k: r.get(k) for k in ("新闻标题", "发布时间", "文章来源")}
                for r in rows
            ]
...
_PUBLIC_MODE = bool(os.environ.get("VR_API_KEY", "").strip()) or os.environ.get(
    "VR_PUBLIC_MODE", ""
...
def _ip_blocked(host: str) -> bool:
    ...
    # IPv4-mapped IPv6（如 ::ffff:127.0.0.1）会被解析成 IPv6Address，
    # 必须再检查其映射的 IPv4，否则公网模式可绕过内网网段判断。
    mapped = getattr(ip, "ipv4_mapped", None)
    candidates = (ip, mapped) if mapped is not None else (ip,)
    if any(candidate in n for candidate in candidates for n in _METADATA_NETS):
        # 云元数据 / 链路本地：SSRF 头号目标，始终禁
        return True
    if _PUBLIC_MODE and any(
        candidate in n for candidate in candidates for n in _PRIVATE_NETS
    ):
        # 公网姿态再禁内网 / 本机
        return True
    return False
...
def _check_base_url(url: str) -> None:
    ...
    本地单用户（未设 VR_API_KEY 且未启用 VR_PUBLIC_MODE）放行 127.0.0.1 等本机地址（方便接本机 Ollama / 网关），只挡 169.254 元数据；
    公网部署（设了 VR_API_KEY 或启用 VR_PUBLIC_MODE）额外禁内网，并解析域名核对，防 DNS 指向内网。"""
    p = urlparse(url or "")
...
def run_chat(cfg: dict, user_messages: list, context: str = "") -> dict:
    ...
    """
    messages = [
        {"role": "system", "content": SYSTEM_PROMPT.format(context=context or "（无）")}
    ]
...
def run_chat_cli(cfg: dict, user_messages: list, context: str = "") -> dict:
    ...
    user = (
        "\n\n".join(m.get("content", "") for m in user_messages if m.get("content"))
        or "（无问题）"
    )
        ...
        for delta in _iter_sse_deltas(resp):
            ...
            for tc in delta.get("tool_calls") or []:
                ...
                if idx is None:
                    ...
                    tc_id = tc.get("id") or ""
                    idx = next(
                        (k for k, v in tool_acc.items() if tc_id and v["id"] == tc_id),
                        None,
                    )
                    if idx is None:
                        idx = (
                            len(tool_acc) if (tc_id or not tool_acc) else max(tool_acc)
                        )
