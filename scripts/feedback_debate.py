...
def feedback_debate_results(
    ...
    for row in rows:
        ...
        print(
            f"   ✅ {symbol} ({signal_date}): 实际 {actual_return:+.1f}% → {'上涨' if was_positive else '下跌'}"
        )
        updated_count += 1
    ...
    if leaderboard:
        ...
        for entry in leaderboard:
            print(
                f"   {entry['role_name']}: 准确率 {entry['accuracy'] * 100:.1f}% ({entry['total_predictions']}次), 权重 {entry['weight']:.2f}"
            )
