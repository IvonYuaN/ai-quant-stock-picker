...
- 默认先在项目内解决问题：代码、测试、脚本、文档、CI、可复现运行参数和用户可见文案。
- 服务器系统配置、宝塔/Nginx/systemd/SSH/防火墙只做只读诊断，不作为常规修复手段。
- 只有发现安全、可用性或数据完整性级别的大风险，才暂停并让仓主判断是否修改系统配置。
- 不把“改服务器配置”当成绕过项目缺陷的捷径；能在仓库内用 guardrail、preflight 或测试挡住的问题必须沉淀回仓库。

## 7. 并行开发资源门禁

- 并行子任务只处理互不重叠的文件范围；默认最多 3 个同时运行的开发/审查子任务。
- 并行是默认执行方式：主线被长任务阻塞时，优先启动不重叠的审查、测试、文档或旁路修复，主线不得无故停等；任务完成或失去价值后立即关闭子任务。
- 子任务必须在隔离工作区或只读审查模式运行，不得操作用户前台应用、浏览器会话或生产服务。
- 单元测试、lint、构建和部署按轻重分级；重型测试不得与多个子任务同时运行，且必须设置超时并在结束后关闭子任务。
