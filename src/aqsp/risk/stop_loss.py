...
class StopLossConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.portfolio_stop >= 0:
            raise ValueError(f"portfolio_stop必须为负数，当前值: {self.portfolio_stop}")
...
class Position:
    ...
    def __post_init__(self) -> None:
        ...
        if self.high_water_mark <= 0:
            raise ValueError(
                f"high_water_mark必须为正数，当前值: {self.high_water_mark}"
            )
...
class StopLossManager:
    ...
    def check_single_stock_stops(
        ...
        for position in positions:
            if position.symbol not in current_prices:
                raise ValueError(f"缺少持仓 {position.symbol} 的价格数据")
    ...
    def check_portfolio_stop(
        ...
        """
        if initial_value <= 0:
            raise ValueError(f"initial_value必须为正数，当前值: {initial_value}")
