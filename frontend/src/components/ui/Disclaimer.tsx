...
export function Disclaimer({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <div className="mt-8 flex items-start gap-2 rounded-lg border border-border/60 bg-muted/20 p-3 text-xs leading-relaxed text-muted-foreground">
      ...
      <span>
        AQSP 页面内容均为<b className="text-foreground">只读研究数据、证据和来源状态</b>；候选仅用于纸面研究与历史复核，<b className="text-foreground">不生成操作指令</b>。请在使用前核对数据日期与来源状态。
      </span>
    </div>
  );
}
