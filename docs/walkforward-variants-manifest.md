...
已收录 27 份历史报告（9 份当前格式 / 18 份旧格式），去重后识别出 **5 个独立变体**。计划新增 **10 个变体**（路径 A 纯权重网格）。
...
**网格维度仅限权重**（momentum_weight, triple_rise_weight），不混入得分门槛（min_total_score）。min_total_score 单独管理，不参与本轮 T×N 矩阵构建。
...
| WF-A05 | sqlite_db | momentum + triple_rise | momentum -0.1, triple_rise +0.1 | 待跑 |
| WF-A06 | sqlite_db | momentum + triple_rise | momentum +0.1, triple_rise -0.1 | 待跑 |
| WF-A07 | sqlite_db | momentum + triple_rise | momentum -0.2（双步长） | 待跑 |
| WF-A08 | sqlite_db | momentum + triple_rise | triple_rise -0.2（双步长） | 待跑 |
| WF-A09 | sqlite_db | momentum + triple_rise | momentum +0.2（双步长） | 待跑 |
| WF-A10 | sqlite_db | momentum + triple_rise | triple_rise +0.2（双步长） | 待跑 |

网格说明：
- 单维度变动 4 个（A01-A04）+ 双维度交换 2 个（A05-A06）+ 双步长 4 个（A07-A10）= 10 个变体
- 所有变体在**权重单纯形**内同台，量纲一致
