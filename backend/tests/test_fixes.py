...
def test_api_key_auth(monkeypatch):
    ...
    assert (
        client.get(
            "/api/quote?codes=abc", headers={"Authorization": "Bearer wrong"}
        ).status_code
        == 401
    )
    ...
    assert (
        client.get(
            "/api/quote?codes=abc", headers={"Authorization": "Bearer sekret"}
        ).status_code
        == 400
    )
...
def test_portfolio_crud_roundtrip(tmp_pf):
    ...
    r = client.post(
        "/api/portfolio/holding", json={"code": "600519", "shares": 100, "cost": 8.0}
    )
    ...
    # 同代码加仓 → 加权平均成本
    client.post(
        "/api/portfolio/holding", json={"code": "600519", "shares": 100, "cost": 12.0}
    )
    ...
    assert (
        client.delete("/api/portfolio/holding?code=600519").json()["data"]["holdings"]
        == []
    )
...
def test_portfolio_add_validation(tmp_pf):
    assert (
        client.post(
            "/api/portfolio/holding", json={"code": "abc", "shares": 1, "cost": 1}
        ).status_code
        == 400
    )
    assert (
        client.post(
            "/api/portfolio/holding", json={"code": "600519", "shares": 0, "cost": 1}
        ).status_code
        == 400
    )
...
def test_portfolio_merge_cost_keeps_4_decimals(tmp_pf):
    client.post(
        "/api/portfolio/holding", json={"code": "510300", "shares": 100, "cost": 1.0001}
    )
    client.post(
        "/api/portfolio/holding", json={"code": "510300", "shares": 100, "cost": 1.0003}
    )
    h = client.get("/api/portfolio").json()["data"]["holdings"][0]
...
def test_emotion_dirty_amount(monkeypatch):
    ...
    monkeypatch.setattr(
        astock, "em_zt_topic_pool", lambda ep, d, sort="": pools.get(ep, [])
    )
    out = market._emotion()
...
def test_stream_tool_calls_without_index(monkeypatch):
        ...
        [  # 第一轮：增量全部不带 index —— 续块无 id、新调用带新 id
            {
                "tool_calls": [
                    {
                        "id": "call_a",
                        "function": {"name": "query_quote", "arguments": '{"codes":'},
                    }
                ]
            },
    ...
    monkeypatch.setattr(
        chat,
        "_exec_tool",
        lambda name, args: (executed.append((name, args)), {"ok": 1})[1],
    )
