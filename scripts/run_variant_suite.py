...
def run_suite(
    ...
    for lookback in sorted(profiles_by_lookback):
        ...
        for profile in profiles_by_lookback[lookback]:
            ...
            payload["independent_signal_days"] = len(
                {
                    fill.date
            ...
            payload["generation"] = profile.generation
            payload["parent_variant_id"] = profile.parent_variant_id
...
def assign_variant_lifecycle(results: list[dict[str, Any]]) -> None:
    """Classify experiment variants without changing formal strategy thresholds."""
    if not results:
        return
    promotion_cutoff = max(3, len(results) // 5)
    elimination_cutoff = max(promotion_cutoff + 1, (len(results) * 3) // 4)
    for item in results:
        rank = int(item.get("rank") or 0)
        signal_days = int(item.get("independent_signal_days") or 0)
...
def _next_generation_proposal(item: dict[str, Any]) -> dict[str, object]:
    ...
    generation = int(item.get("generation") or strategy.get("generation") or 1) + 1
    return {
