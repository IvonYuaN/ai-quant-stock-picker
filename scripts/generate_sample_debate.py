...
def generate_sample_debate_data():
    """生成模拟的辩论结果数据"""
    agent_roles = [
        "技术多头", "基本面空头", "风险控制", "板块轮动",
        "政策分析", "融资融券", "北向资金", "散户情绪"
    ]
    agent_role_keys = ["bull", "bear", "risk_control", "sector_leader",
                      "policy_sensitive", "margin_trading", "northbound", "retail_mood"]
    ...
    for symbol_info in symbols:
        ...
        for round_num in range(1, 3):
            ...
            rounds.append({
                "round_num": round_num,
                "summary": f"第{round_num}轮辩论完成，各Agent从不同角度分析了{name}的投资价值",
                "opinions": opinions,
                "cross_opinions": {}
            })
        ...
        if bull_count > bear_count + 1:
            final_consensus = f"多Agent辩论后，{bull_count}个看多，{bear_count}个看空，{neutral_count}个中性，整体看多，建议关注"
        elif bear_count > bull_count + 1:
            final_consensus = f"多Agent辩论后，{bear_count}个看空，{bull_count}个看多，{neutral_count}个中性，整体偏谨慎，建议观望"
        else:
            final_consensus = f"多Agent辩论后，{bull_count}个看多，{bear_count}个看空，{neutral_count}个中性，观点分化，保持原评级"
        ...
        results.append({
            "debate_id": f"debate_{symbol}_{datetime.now().strftime('%Y%m%d%H%M%S')}",
            "symbol": symbol,
            "name": name,
            ...
            "regime": "震荡偏多",
            "data_source": "multi",
            "related_signal_date": datetime.now().date().isoformat(),
            "rounds": rounds,
            "final_consensus": final_consensus,
            "final_vote": final_vote,
            "adjustment_reason": f"多头{bull_count}票 vs 空头{bear_count}票，辩论建议{'上调' if recommended == 'raise' else '下调' if recommended == 'lower' else '维持'}评分至 {adjusted_score}",
            "risk_warnings": [
                "需关注大盘系统性风险",
                "业绩波动风险",
                "政策调整风险"
