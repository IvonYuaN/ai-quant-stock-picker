...
if [[ -f "$AQSP_RELEASE_MANIFEST" ]]; then
    AQSP_RELEASE_COMMIT="$(${AQSP_BOOTSTRAP_PYTHON:-python3} - "$AQSP_RELEASE_MANIFEST" <<'PY'
import json
import sys
from pathlib import Path
...
export_runtime_path AQSP_VARIANT_CURSOR_PATH data/runtime/variant_results_cursor.json
export_runtime_path AQSP_VARIANT_REFRESH_STATUS data/runtime/variant_refresh_status.json
export_runtime_path AQSP_VARIANT_REFRESH_LOCK data/.locks/variant-results-refresh.lock
export_runtime_path AQSP_VARIANT_LOG_DIR data/logs/variants
