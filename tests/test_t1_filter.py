...
def test_get_yesterday_buys_returns_signals(tmp_path):
    """返回昨日 signal_date 匹配的 symbol"""
    ledger_path = tmp_path / "ledger.jsonl"
    records = [
        {"signal_date": "2026-05-26", "symbol": "600519", "status": "pending"},
        {"signal_date": "2026-05-26", "symbol": "000858", "status": "pending"},
        {"signal_date": "2026-05-25", "symbol": "000001", "status": "pending"},
    ]
...
def test_filter_t1_held_skips_invalid_lines(tmp_path):
    """优雅处理格式错误的 JSON"""
    ledger_path = tmp_path / "ledger.jsonl"
    content = """{"signal_date": "2026-05-26", "symbol": "600519", "status": "pending"}
invalid json line
{"signal_date": "2026-05-26", "symbol": "000858", "status": "pending"}
"""
...
def test_get_yesterday_buys_deduplicates(tmp_path):
    ...
    records = [
        {"signal_date": "2026-05-26", "symbol": "600519", "status": "pending"},
        {"signal_date": "2026-05-26", "symbol": "600519", "status": "executed"},
    ]
    ledger_path.write_text("\n".join(json.dumps(r) for r in records))
    result = get_yesterday_buys(ledger_path, date(2026, 5, 27))
