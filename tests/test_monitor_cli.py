...
def test_run_monitor_fails_when_all_checks_skipped_without_flag(
    monkeypatch, capsys
) -> None:
    class FakeChecker:
        def __init__(self, config_path: str) -> None:
        ...
        def check_all(self) -> list[MonitorResult]:
            return [
                MonitorResult(
                    "skipped_a", False, "warning", "数据缺失，跳过", skipped=True
                ),
                MonitorResult(
                    "skipped_b", False, "critical", "账本缺失，跳过", skipped=True
                ),
            ]
    ...
    assert "因数据不可用被跳过" in out
    assert "零监控" in out


def test_run_monitor_allows_all_checks_skipped_with_flag(monkeypatch, capsys) -> None:
    class FakeChecker:
        def __init__(self, config_path: str) -> None:
            self.config_path = config_path
    ...
    assert exit_code == 0
    assert "冒烟运行" in capsys.readouterr().out


def test_run_monitor_reports_smoke_when_partial_skips_no_triggers(
    monkeypatch, capsys
) -> None:
    # 真实 CI 场景：部分是真实结果（如熔断/数据源），部分因无数据被跳过。
    # 此时不应报“所有监控项正常”（假绿），而应明确“冒烟通过 + N 项跳过”。
    class FakeChecker:
        def __init__(self, config_path: str) -> None:
            self.config_path = config_path
        ...
        def check_all(self) -> list[MonitorResult]:
            return [
                MonitorResult("circuit_breaker", False, "critical", "未触发"),
                MonitorResult(
                    "skipped_a", False, "warning", "数据缺失，跳过", skipped=True
                ),
            ]
    ...
    assert exit_code == 0
    out = capsys.readouterr().out
    assert "监控冒烟通过" in out
    assert "所有监控项正常" not in out
