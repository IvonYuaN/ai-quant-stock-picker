...
STOP_SETTINGS = {
    ...
    "hold_3d":      {"stop_loss_pct": 1.0,  "take_profit_pct": 1.0},  # 纯持有 horizon（无止损无止盈）
}
...
def main() -> int:
    ...
    rows: list[dict] = []
    for name, cfg in STOP_SETTINGS.items():
        ...
        print(f"[run ] {name}: stop_loss_pct={cfg['stop_loss_pct']} tp={cfg['take_profit_pct']} ...", flush=True)
        ...
        rows.append(
            {
                ...
                "avg_period_return": (sum(period_returns) / len(period_returns)) if period_returns else None,
                "regime_winrates": dict(res.regime_winrates or {}),
                "period_returns": period_returns,
            }
        )
        print(
            f"[done] {name}: total={_fmt_pct(rows[-1]['overall_total_return'])} "
            f"sharpe={res.overall.sharpe_ratio:.2f} "
            f"win={_fmt_pct(rows[-1]['overall_win_rate'])} "
...
def _write_report(args: argparse.Namespace, rows: list[dict]) -> None:
    ...
    lines.append(f"- 样本区间: `{args.start}` ~ `{args.end}`（与 item 2 gate 同一样本）")
    lines.append("- 配置: WF-001（top_n=10, horizon=3, lookback=60, momentum/triple_rise=0.3/0.3）")
    lines.append(f"- 训练/测试/Purge: {TRAIN_DAYS}/{TEST_DAYS}/{PURGE_DAYS} 天")
    lines.append("- 唯一被比较维度: `stop_loss_pct`（出场规则）；三档统一 `take_profit_pct=1.0`（止盈已关闭，仅比止损政策）\n")

    lines.append("## 汇总对比\n")
    lines.append("| 出场规则 | stop_loss_pct | OOS 总收益 | Sharpe | 总胜率 | 周期胜率 | PBO | DSR | 周期数 |")
    lines.append("|---|---|---|---|---|---|---|---|---|")
    for r in rows:
    ...
    for r in rows:
        cells = [r["name"]] + [f"{_fmt_pct(r['regime_winrates'].get(rg))}" for rg in regimes]
    ...
    lines.append("\n## 结论（证据，非决策）\n")
    lines.append("- ⚠️ **口径警示**：本脚本每个出场规则都是**单序列**回测（`n_variants=1`），因此")
    lines.append("  - `PBO` 恒为 N/A —— 单序列无法做 CSCV（宪法 §17.7）；本实验**不作过拟合判级**；")
    lines.append("  - `DSR` 未做多重检验修正（`n_trials=1`），**不可与 gate 的 grid CSCV 读数横向比较**；")
    lines.append("  - 本实验的有效结论只来自 **同一 OOS 样本下的相对比较**：OOS 总收益 / Sharpe / 胜率。")
    lines.append("- 本表为同一 OOS 样本、仅改变出场规则（止损）的对比，可用于佐证「8% 硬止损 vs 持有 3 天」的取舍。")
    lines.append("- 实验设计：三档统一关闭止盈（take_profit_pct=1.0），故比较的是「纯止损政策」对 OOS 的影响；")
    lines.append("  绝对收益与 live（含默认止盈）不完全一致，但相对差异干净隔离了止损维度。")
    lines.append("- `current_soft`(None) 即 live 现状（动态 ATR 软止损）；`hard_8pct` 为固定 8% 硬止损；`hold_3d`(1.0) 为纯持有 horizon。")
