...
if ! [[ "$INTRADAY_RUN_TIMEOUT_SECONDS" =~ ^[0-9]+$ ]] || [ "$INTRADAY_RUN_TIMEOUT_SECONDS" -le 0 ]; then
    log "盘中运行超时配置无效(${INTRADAY_RUN_TIMEOUT_SECONDS})，使用 1200 秒"
    INTRADAY_RUN_TIMEOUT_SECONDS="1200"
fi
...
if ! [[ "$INTRADAY_MAX_UNIVERSE" =~ ^[0-9]+$ ]]; then
    log "盘中最大扫描范围无效(${INTRADAY_MAX_UNIVERSE})，使用完整实时流动性池"
    INTRADAY_MAX_UNIVERSE="0"
elif [ "$INTRADAY_MAX_UNIVERSE" -gt 0 ] && [ "$INTRADAY_MAX_UNIVERSE" -gt 120 ] && ! is_truthy "${AQSP_INTRADAY_ALLOW_HEAVY_UNIVERSE:-false}"; then
...
# failed and prevented cross-market evidence from reaching the debate gate.
