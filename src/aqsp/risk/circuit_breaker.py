    ...
    def check(
        ...
        if monthly_pnl_pct <= -self.config.monthly_loss_pct:
            return self._trigger(
                "monthly",
                monthly_pnl_pct,
                f"月度组合亏损 {monthly_pnl_pct:.2%} 触及 {self.config.monthly_loss_pct:.1f}% 止损线",
                daily_pnl_pct,
                weekly_pnl_pct,
            )
        ...
        if weekly_pnl_pct <= -self.config.weekly_loss_pct:
            return self._trigger(
                "weekly",
                weekly_pnl_pct,
                f"周度组合亏损 {weekly_pnl_pct:.2%} 触及 {self.config.weekly_loss_pct:.1f}% 止损线",
                daily_pnl_pct,
                weekly_pnl_pct,
            )
        ...
        if daily_pnl_pct <= -self.config.daily_loss_pct:
            return self._trigger(
                "daily",
                daily_pnl_pct,
                f"单日组合亏损 {daily_pnl_pct:.2%} 触及 {self.config.daily_loss_pct:.1f}% 止损线",
                daily_pnl_pct,
                weekly_pnl_pct,
            )
