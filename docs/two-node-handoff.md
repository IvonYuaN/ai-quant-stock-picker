...
**实测耗时约束（3y / `stable_plus` / 20 期，runner 独跑）**：单期约 **33 min** → 跑满 20 期约 **11h**。
2026-09-19 首轮用 `TIMEOUT_SEC=36000`（10h）→ **只跑到 18/20 期就被杀**（`status=timeout`、`exit=124`、无 gate 产物）。
故 `TIMEOUT_SEC` 已上调为 **57600（16h）**，`runner_gate.sh` 默认值与 cron 显式传值保持一致；拉取时间也相应从 13:00 推到 18:00。
