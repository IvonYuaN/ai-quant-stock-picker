...
def main() -> int:
    ...
    parser.add_argument(
        "--batch-size",
        type=int,
        default=int(os.getenv("AQSP_INTRADAY_BATCH_SIZE", "32")),
    )
    parser.add_argument(
        "--min-avg-amount",
        type=float,
    ...
    if args.commit:
        ...
        return 0
    if args.fail:
        skipped = cursor.fail_current(args.fail, max_retries=args.max_retries)
        print("skipped" if skipped else "retry", file=sys.stderr)
        return 0
