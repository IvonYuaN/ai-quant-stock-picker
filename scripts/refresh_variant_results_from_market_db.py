...
def load_focus_cohort(path: Path | None) -> tuple[dict[str, object], ...]:
    """Read today's candidate and debate links from the bounded home snapshot."""
    if path is None:
        return ()
    try:
    ...
    debates = payload.get("debates") if isinstance(payload, dict) else None
    ...
    for candidate in candidates:
        ...
        if not symbol:
            continue
        debate = debates_by_symbol.get(symbol, {})
        cohort.append(
            {
                "symbol": symbol,
                "display_name": str(candidate.get("display_name") or symbol),
                "strategies": list(candidate.get("strategies") or ()),
                "discussion_conclusion": str(debate.get("conclusion") or ""),
                "risk_gate": str(debate.get("primary_risk_gate") or ""),
                "next_trigger": str(debate.get("next_trigger") or ""),
            }
        )
    return tuple(cohort)


def prioritize_focus_symbols(
    batch: tuple[MarketSymbol, ...],
    eligible: tuple[MarketSymbol, ...],
    focus_cohort: tuple[dict[str, object], ...],
) -> tuple[MarketSymbol, ...]:
    """Guarantee current discussed candidates enter the bounded experiment batch."""
    if not focus_cohort:
        return batch
    by_symbol = {item.symbol: item for item in eligible}
    priority = [
        by_symbol[symbol]
        for symbol in (str(item.get("symbol") or "") for item in focus_cohort)
        if symbol in by_symbol
    ]
    seen = {item.symbol for item in priority}
    combined = [*priority, *(item for item in batch if item.symbol not in seen)]
    return tuple(combined[: len(batch)])


def attach_discussion_links(
    payload: dict[str, object], focus_cohort: tuple[dict[str, object], ...]
) -> None:
    strategy_modes = {
        "ma_pullback": {"pullback", "volume_dry_pullback"},
        "low_vol_trend": {"low_vol", "trend", "defensive_range"},
        "n_rebound": {"reversion", "kdj_rebound"},
        "rps_momentum": {"relative_strength", "trend"},
        "volume_breakout": {"volume_breakout", "breakout"},
    }
    payload["focus_cohort"] = list(focus_cohort)
    ...
    for variant in variants:
        ...
        variant["discussion_links"] = [
            item
            for item in focus_cohort
            if any(
                mode in strategy_modes.get(str(strategy), set())
...
def evolution_profiles(
    base_profiles: tuple[VariantProfile, ...],
    previous: dict[str, object] | None,
) -> tuple[VariantProfile, ...]:
    """Carry the prior generation forward and replace only audited losers."""
    raw_variants = previous.get("variants") if isinstance(previous, dict) else None
    ...
    for item in raw_variants:
        ...
        variant_id = str(item.get("variant_id") or strategy.get("id") or "").strip()
        if not variant_id:
            return base_profiles
        ...
        if item.get("lifecycle_status") == "淘汰" and isinstance(
            ...
            generation = int(proposal.get("generation") or generation + 1)
            variant_id = f"{parent_variant_id}__g{generation}"
            entry_return_pct = float(
                proposal.get("entry_return_pct") or entry_return_pct
            )
        ...
        evolved.append(
            VariantProfile(
                ...
                mode=str(strategy.get("mode") or "trend"),
...
def merge_stage_into_payload(
    ...
    assign_variant_lifecycle(ranked)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--focus-snapshot", type=Path)
    ...
    try:
        with (
            ...
            with temp_db_context as temporary:
                ...
                all_profiles = evolution_profiles(
                    generate_variant_profiles(
            ...
            attach_discussion_links(payload, focus_cohort)
            payload["universe"] = {
