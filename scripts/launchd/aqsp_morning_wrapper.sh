...
RUN_TIMEOUT_SECONDS="${AQSP_MORNING_TIMEOUT_SECONDS:-3600}"
...
export AQSP_RUN_TASK_ID="morning"

# ============================ 执行 ============================

log "运行早盘打板策略 (timeout=${RUN_TIMEOUT_SECONDS}s)..."
if run_with_timeout "$RUN_TIMEOUT_SECONDS" "${PYTHON_BIN}" -m aqsp morning-breakout --notify 2>&1; then
    :
else
    RUN_STATUS=$?
    log "早盘策略运行失败 (exit ${RUN_STATUS})"
    touch "$ERROR_FILE"
    exit "$RUN_STATUS"
fi
